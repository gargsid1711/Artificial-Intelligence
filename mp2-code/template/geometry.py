...
def doesArmTouchObjects(armPosDist, objects, isGoal=False):
    ...
    for arm in armPosDist:
        ...
        for obj in objects:
            current_obj = np.array([obj[0], obj[1]])
            radius = obj[2]
            x = d_vector.dot(d_vector)
            y = 2 * d_vector.dot(start - current_obj)
            z = start.dot(start) + current_obj.dot(current_obj) - 2 * start.dot(current_obj) - (radius + padding) ** 2
            disc = y ** 2 - 4 * x * z
            if disc < 0:
                continue
...
def doesArmTipTouchGoals(armEnd, goals):
    ...
    """
    for g in goals:
        e_dist = np.sqrt((armEnd[0]-g[0])**2 + (armEnd[1]-g[1])**2)
        #distance between arm tip and center of goal
...
def isArmWithinWindow(armPos, window):
    ...
    for arm in armPos:
        ...
        x_e = arm[1][0]
        y_e = arm[1][1]
        if (x_s < 0 or x_s > width) or (y_s < 0 or y_s > height) or (x_e < 0 or x_e > width) or (y_e < 0 or y_e > height):
            return False
    return True
